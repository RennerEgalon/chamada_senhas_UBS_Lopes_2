...
async function atualizarContadorFirebaseSeMaior(tipo, numero, idColuna) {
  const ref = firebase.database().ref('maioresSenhasPorColuna/' + idColuna);
...
function criarBotao(idColuna, texto, classe) {
  ...
  botao.onclick = () => {
    ...
    if (!maioresSenhasPorColuna[idColuna] || numeroSenha > maioresSenhasPorColuna[idColuna]) {
      ...
      atualizarContadorFirebaseSeMaior(tipo, numeroSenha, idColuna);
...
async function chamarSenhaSincronizada(tipo, guiche) {
  ...
  if (guiche === 1) {
    idColuna = tipo === 'normal' ? 'coluna-normal-guiche1' : 'coluna-preferencial-guiche1';
  } else if (guiche === 2) {
    idColuna = tipo === 'normal' ? 'coluna-normal-guiche2' : 'coluna-preferencial-guiche2';
  } else if (guiche === 3) {
    idColuna = tipo === 'normal' ? 'coluna-normal-posconsulta' : 'coluna-preferencial-posconsulta';
  } else {
    console.error('Guichê inválido:', guiche);
    return;
  }
