// Firebase já deve estar inicializado no index.html
...
async function limparSenhas() {
  ...
  for (let key in ultimasSenhasChamadas) {
    ultimasSenhasChamadas[key] = 0;
  }
...
async function chamarSenhaSincronizada(tipo, guiche) {
  ...
  // Sempre usar o maior entre Firebase e local
  let proximaSenha = Math.max(contador, ultimaSenha) + 1;

  // Se contador for zero (após reset), garantir que proxima seja 1
  if (proximaSenha <= 1) {
    proximaSenha = 1;
  }
